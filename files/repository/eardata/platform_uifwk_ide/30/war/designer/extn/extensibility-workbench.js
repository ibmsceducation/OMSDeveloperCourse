...
*/		
	...
	, showOptions: function(e, component, screen, editor, componentConfig, overlays, extensionContext) {
		...
		while(!(def instanceof sci.ide.cmpdefs.Panel || def instanceof sci.ide.cmpdefs.tb.Toolbar) && containerComponent) {
    ...
    , findEventContext: function(e) {
		...
		if(cmp && cmp.ownerCt) {
			var parentCt = cmp.ownerCt;
			var hasFileUploadWidget = false;
			while(parentCt && !hasFileUploadWidget) {
				if(parentCt.xtype) {
					var xtype = parentCt.xtype;
					var def = sci.ide.RegDefObject.get(xtype);
					if(def && def instanceof sci.ide.cmpdefs.FileUploadWidget) {
						hasFileUploadWidget = true;
						cmp = parentCt;
					} else {
						parentCt = parentCt.ownerCt;
					}
				} else {
					parentCt = parentCt.ownerCt;
				}
